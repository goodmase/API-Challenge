...
export class MessageController {
  constructor(
    @Inject('JOB_SERVICE') private readonly client: ClientProxy,
    private readonly messageService: InMemoryDBService<MessageEntity>,
  ) {}
  ...
  async addMessage(@Body() message: CreateMessageDto) {
    ...
    const msg = this.messageService.create({
  ...
  async updateMessage(message: MessageEntity) {
    console.log(
      `Received updated status: ${message.status} for job_id ${message.id}`,
    );
